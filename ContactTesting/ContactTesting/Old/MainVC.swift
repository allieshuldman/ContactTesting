...
class MainVC: UIViewController {
  ...
  func setUpView() {
    ...
    historyButton.setTitle("View Change History", for: .normal)
    historyButton.addTarget(self, action: #selector(didTapViewChangeHistory), for: .touchUpInside)
  ...
  override func viewDidLayoutSubviews() {
    ...
    numberOfContactsInMemoryLabel.frame =  CGRect(
     x: UIConstants.leftInset,
     y: historyButton.frame.maxY + UIConstants.topSpacing,
     width: view.frame.width - 2 * UIConstants.leftInset,
  ...
  @objc func didTapViewChangeHistory() {
    let historySelectionVC = HistorySelectionVC()
    navigationController?.pushViewController(historySelectionVC, animated: true)
